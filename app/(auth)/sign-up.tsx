...
export default function SignUpScreen() {
    ...
    try {
      ...
      const clerkError =
        err && typeof err === "object" && "errors" in err
          ...
          : undefined;
      setError(clerkError ?? "Unable to create your account.");
    } finally {
  ...
  const handleSocialSignUp = async (
    ...
    try {
      ...
      setError("The social sign-up flow is incomplete. Please try again.");
    } catch (err) {
      const clerkError =
        err && typeof err === "object" && "errors" in err
          ? (err as { errors?: { message?: string }[] }).errors?.[0]?.message
          : undefined;
      setError(clerkError ?? "Unable to continue with that provider.");
    } finally {
      setIsSubmitting(false);
    }
  };
  ...
  const handleVerify = async () => {
    ...
    try {
      ...
      setError("Your email verification is still pending.");
    } catch (err) {
      const clerkError =
        err && typeof err === "object" && "errors" in err
          ? (err as { errors?: { message?: string }[] }).errors?.[0]?.message
          : undefined;
      setError(clerkError ?? "Unable to verify your email address.");
    } finally {
      setIsSubmitting(false);
    }
  };
    ...
    <View className="flex-1 bg-background px-5 pt-20">
      ...
      <View className="mt-6 items-center">
        ...
        <Text className="mt-2 max-w-[320px] text-center text-base font-sans-medium text-muted-foreground">
          Join and keep your subscriptions in sync
        </Text>
      </View>
      ...
      <View className="mt-8 rounded-3xl border border-border bg-card p-5">
        <View className="gap-4">
          ...
          <Pressable
            ...
            className={
              isSubmitting
                ? "auth-button auth-button-disabled px-4"
                : "auth-button px-4"
            }
          >
